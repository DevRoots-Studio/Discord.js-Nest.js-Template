...
export function startBridgeServer(client: DiscordClient): void {
  ...
  app.post('/bridge/send-message', auth, async (req: Request, res: Response) => {
    try {
      const { channelId, content } = req.body as { channelId?: string; content?: string };
      if (!channelId || typeof content !== 'string') {
        res.status(400).json({ success: false, error: 'channelId and content required' });
        return;
      }
      const channel = await client.channels.fetch(channelId);
      if (!channel?.isTextBased() || !('send' in channel)) {
        res.status(400).json({ success: false, error: 'Channel not found or not text channel' });
        return;
      }
      const msg = await (channel as import('discord.js').TextChannel).send(content);
